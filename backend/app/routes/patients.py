from flask import Blueprint, request, jsonify
from flask_jwt_extended import jwt_required
from app.utils.security import get_current_user
from app.services.patient_service import PatientService
from app.extensions import limiter
from app.audit.decorators import audit

patients_bp = Blueprint("patients", __name__)


@patients_bp.route("/patients", methods=["POST"])
@limiter.limit("100 per hour")
@jwt_required()
@audit(action="create", resource="patient")
def create_patient():
    data = request.get_json() or {}
    current_user = get_current_user()
    patient = PatientService.create_patient(data, current_user)
    
    details = {
        "patient_id": patient.get("id"),
        "name": f"{patient.get('first_name')} {patient.get('last_name')}"
    }
    return jsonify(patient), 201, details


@patients_bp.route("/patients", methods=["GET"])
@limiter.limit("100 per minute")
@jwt_required()
@audit(action="list", resource="patient")
def list_patients():
    current_user = get_current_user()

    def str_to_bool(value):
        if value is None:
            return None
        return value.lower() in ("true", "1", "yes")

    filters = {
        "identification_number": request.args.get("identification_number"),
        "first_name": request.args.get("first_name"),
        "last_name": request.args.get("last_name"),
        "has_eeg_records": str_to_bool(request.args.get("has_eeg_records")),
        "has_pending_eeg": str_to_bool(request.args.get("has_pending_eeg")),
    }
    patients = PatientService.list_patients(filters, current_user)

    details = {"filters": filters}
    return jsonify(patients), 200, details


@patients_bp.route("/patients/<uuid:patient_id>", methods=["GET"])
@limiter.limit("100 per minute")
@jwt_required()
@audit(action="get", resource="patient")
def get_patient(patient_id):
    current_user = get_current_user()
    patient = PatientService.get_patient(patient_id, current_user)

    details = {"patient_id": str(patient_id)}
    return jsonify(patient), 200, details


@patients_bp.route("/patients/<uuid:patient_id>", methods=["PUT"])
@limiter.limit("50 per hour")
@jwt_required()
@audit(action="update", resource="patient")
def update_patient(patient_id):
    data = request.get_json() or {}
    current_user = get_current_user()
    patient = PatientService.update_patient(patient_id, data, current_user)
    
    details = {
        "patient_id": str(patient_id),
        "fields_updated": list(data.keys())
    }
    return jsonify(patient), 200, details


@patients_bp.route("/patients/<uuid:patient_id>", methods=["DELETE"])
@limiter.limit("50 per hour")
@jwt_required()
@audit(action="delete", resource="patient")
def delete_patient(patient_id):
    current_user = get_current_user()
    patient = PatientService.delete_patient(patient_id, current_user)
    
    details = {
        "patient_id": str(patient_id),
        "name": f"{patient.get('first_name')} {patient.get('last_name')}"
    }
    return jsonify({"message": f"Patient {patient['id']} deleted"}), 200, details
