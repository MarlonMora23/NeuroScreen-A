...
class TestUploadEeg:
    ...
    def test_upload_invalid_patient_id_format(self, client, user_headers, parquet_file):
        file_data, filename = parquet_file
        response = client.post(
            "/api/eeg-records/upload",
            data={
    ...
    def test_upload_file_too_large(self, client, user_headers, sample_patient):
        # Crear un archivo grande simulado
        large_data = io.BytesIO(b"x" * (1024 * 1024 * 100))  # 100 MB, asumiendo límite menor
        response = client.post(
            "/api/eeg-records/upload",
            data={
                "patient_id": str(sample_patient.id),
                "file": (large_data, "large.parquet", "application/octet-stream")
            },
            headers=user_headers,
            content_type="multipart/form-data"
        )
    ...
    def test_upload_file_without_filename(self, client, user_headers, sample_patient):
        # Archivo sin filename
        ...
        response = client.post(
            ...
            data={
                "patient_id": str(sample_patient.id),
                "file": (file_data, "", "application/octet-stream")
            },
            headers=user_headers,
            content_type="multipart/form-data"
        )
    ...
    def test_upload_corrupted_file(self, client, user_headers, sample_patient):
        # Archivo parquet corrupto
        corrupted_data = io.BytesIO(b"not a parquet file")
        response = client.post(
            "/api/eeg-records/upload",
            data={
                "patient_id": str(sample_patient.id),
                "file": (corrupted_data, "corrupted.parquet", "application/octet-stream")
            },
            headers=user_headers,
            content_type="multipart/form-data"
        )
    ...
    def test_upload_admin_can_upload_to_others_patient(self, client, admin_headers, user_headers, sample_patient, parquet_file):
        # Admin puede subir a paciente de otro usuario
        response = upload_eeg(client, admin_headers, sample_patient.id, parquet_file)
        assert response.status_code == 202
...
class TestListEegRecords:
    ...
    def test_filter_by_invalid_patient_id(self, client, user_headers):
        response = client.get("/api/eeg-records?patient_id=invalid-uuid", headers=user_headers)
        assert response.status_code == 400

    def test_list_eeg_requires_auth(self, client, sample_patient, parquet_file):
        response = client.get("/api/eeg-records", headers={})
        assert response.status_code == 401
...
class TestGetEegRecord:
    ...
    def test_get_record_with_invalid_uuid(self, client, user_headers):
        response = client.get("/api/eeg-records/invalid-uuid", headers=user_headers)
        assert response.status_code == 404

    def test_admin_can_get_another_users_record(self, client, admin_headers, user_headers, sample_patient, parquet_file):
        # Upload como usuario regular
        r = upload_eeg(client, user_headers, sample_patient.id, parquet_file)
        eeg_id = r.get_json()["id"]

        # Admin puede acceder
        response = client.get(f"/api/eeg-records/{eeg_id}", headers=admin_headers)
        assert response.status_code == 200
        assert response.get_json()["id"] == eeg_id
...
class TestEegStatus:
    ...
    def test_status_invalid_uuid(self, client, user_headers):
        response = client.get("/api/eeg-records/invalid-uuid/status", headers=user_headers)
        assert response.status_code == 404

    def test_admin_can_get_status_of_others_record(self, client, admin_headers, user_headers, sample_patient, parquet_file):
        r = upload_eeg(client, user_headers, sample_patient.id, parquet_file)
        eeg_id = r.get_json()["id"]

        response = client.get(f"/api/eeg-records/{eeg_id}/status", headers=admin_headers)
        assert response.status_code == 200
        data = response.get_json()
        assert "status" in data

    def test_status_requires_auth(self, client, sample_patient, parquet_file):
        response = client.get(f"/api/eeg-records/{uuid.uuid4()}/status")
        assert response.status_code == 401
...
class TestEegVisualizations:
    ...
    def test_visualizations_invalid_uuid(self, client, user_headers):
        response = client.get("/api/eeg-records/invalid-uuid/visualizations", headers=user_headers)
        assert response.status_code == 404

    def test_visualizations_no_prediction(self, client, user_headers):
        # Crear un record sin prediction (simular)
        # Esto es difícil de simular directamente, pero asumimos que si no hay prediction, retorna 404
        response = client.get(f"/api/eeg-records/{uuid.uuid4()}/visualizations", headers=user_headers)
        assert response.status_code == 404

    def test_admin_can_access_others_visualizations(self, client, admin_headers, user_headers, sample_patient, parquet_file):
        r = upload_eeg(client, user_headers, sample_patient.id, parquet_file)
        eeg_id = r.get_json()["id"]
    ...
    def test_visualization_without_channels_filter(self, client, user_headers, sample_patient, parquet_file):
        r = upload_eeg(client, user_headers, sample_patient.id, parquet_file)
        eeg_id = r.get_json()["id"]
...
class TestDeleteEegRecord:
    ...
    def test_delete_invalid_uuid(self, client, user_headers):
        response = client.delete("/api/eeg-records/invalid-uuid", headers=user_headers)
        assert response.status_code == 404

    def test_delete_nonexistent_record(self, client, user_headers):
        response = client.delete(f"/api/eeg-records/{uuid.uuid4()}", headers=user_headers)
        assert response.status_code == 404

    # Nota: Para probar delete en PROCESSING, necesitaríamos modificar el estado manualmente en la DB,
    # pero eso requiere acceso directo al modelo, lo cual no es ideal en tests de integración.
    # Se puede asumir que está cubierto si el código lo maneja.

    def test_admin_can_delete_other_users_record(self, client, admin_headers, user_headers, sample_patient, parquet_file):
        r = upload_eeg(client, user_headers, sample_patient.id, parquet_file)
        eeg_id = r.get_json()["id"]
    ...
    def test_delete_requires_auth(self, client, sample_patient, parquet_file):
        response = client.delete(f"/api/eeg-records/{uuid.uuid4()}")
        assert response.status_code == 401
...
class TestListByPatient:
    ...
    def test_list_by_invalid_patient_uuid(self, client, user_headers):
        response = client.get("/api/patients/invalid-uuid/eeg-records", headers=user_headers)
        assert response.status_code == 404

    def test_list_by_nonexistent_patient(self, client, user_headers):
        response = client.get(f"/api/patients/{uuid.uuid4()}/eeg-records", headers=user_headers)
        assert response.status_code == 404

    def test_admin_sees_all_records_by_patient(self, client, admin_headers, user_headers, sample_patient, parquet_file):
        # Upload como usuario regular
        upload_eeg(client, user_headers, sample_patient.id, parquet_file)

        # Admin puede ver todos los records del paciente
        response = client.get(
            f"/api/patients/{sample_patient.id}/eeg-records",
            headers=admin_headers
        )
    ...
    def test_user_cannot_see_other_users_records_by_patient(
        self, client, user_headers, another_user_headers, sample_patient, parquet_file
    ):
        # sample_patient pertenece a user_headers, no a another_user_headers
        upload_eeg(client, user_headers, sample_patient.id, parquet_file)
        ...
        # another_user no puede acceder a registros de patient que no le pertenece
        assert response.status_code == 403